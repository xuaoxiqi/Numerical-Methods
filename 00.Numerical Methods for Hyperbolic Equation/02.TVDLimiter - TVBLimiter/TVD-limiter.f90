...
!!!    Using Godunov numerical flux, with TVD Limiter
...
!!!    Ao Xu, Profiles: <http://www.linkedin.com/pub/ao-xu/30/a72/a29>
       ...
       write(*,*)
       write(*,*) '3rd FVM SCHEME(TVD Limiter):'
       write(*,*) '****************************************'
...
101    format('Title="Burgers Equation(TVD Limiter)"')
...
!       write(*,*) 'x0=',X(N),'u(i)=',u_avg(0)
       ...
       do i=0,N-1
              ...
              call TVD_Limiter(ul,ur,u_avg(MOD(i-1+N,N)),u_avg(i),u_avg(MOD(i+1,N)),u_avg(MOD(i+2,N)))
       ...
       subroutine TVD_Limiter(ul,ur,u_avg_j0,u_avg_j1,u_avg_j2,u_avg_j3)
       implicit none
       real(8) :: ul, ur, u_avg_j0, u_avg_j1, u_avg_j2, u_avg_j3, u_tilde, u_dtilde

       u_tilde = ul-u_avg_j1
       u_dtilde = u_avg_j2-ur

       !if((u_tilde*(u_avg_j2-u_avg_j1).GT.0.0).AND.(u_tilde*(u_avg_j1-u_avg_j0).GT.0.0)) then
       if ((SIGN(1.0d0,u_tilde).EQ.SIGN(1.0d0,u_avg_j2-u_avg_j1)).AND.(SIGN(1.0d0,u_tilde).EQ.SIGN(1.0d0,u_avg_j1-u_avg_j0))) then
              u_tilde = MIN(ABS(u_tilde),ABS(u_avg_j2-u_avg_j1),ABS(u_avg_j1-u_avg_j0))*SIGN(1.0d0,u_tilde)
       else
              u_tilde = 0.0d0
       endif

       !if((u_dtilde*(u_avg_j3-u_avg_j2).GT.0.0).AND.(u_dtilde*(u_avg_j2-u_avg_j1).GT.0.0)) then
       if ((SIGN(1.0d0,u_dtilde).EQ.SIGN(1.0d0,u_avg_j3-u_avg_j2)).AND.(SIGN(1.0d0,u_dtilde).EQ.SIGN(1.0d0,u_avg_j2-u_avg_j1))) then
