...
103     format('zone',1x,'i=',1x,i5,2x,'f=point')
        ...
        do i=1,N
            if(u_r(i).LE.u_l(i+1)) then
                f(i) = MIN(0.5d0*u_r(i)*u_r(i),0.5d0*u_l(i+1)*u_l(i+1))
                if(u_l(i+1)*u_r(i).LT.0.0d0) f(i) = 0.0d0
            else
                f(i) = MAX(0.5d0*u_l(i+1)*u_l(i+1),0.5d0*u_r(i)*u_r(i))
            endif
        enddo
        f(0) = f(N-1)
